...
def train(args, data_info):
    ...
    for step in range(args.n_epoch):
        ...
        #epoch 종료 후 training result check
...
def topk_recommendation(args, model, infer_data, user_triple_set, item_triple_set):
    topK = 10
    logging.info('calculating topK recommendation ...')
    
    item_set = set(infer_data[:,1].tolist())
    user_record = _get_user_record(args, infer_data, True)
    user_list = list(set(user_record.keys()))
    
    model.eval()
    logging.info('ready for topK recommendation ...')
    
    for user in user_list:
        item_list = list(item_set)
        item_score_map = dict()
        
        input_data = _get_topk_feed_data(user, item_list)
        scores = model(*_get_feed_data(args, input_data, user_triple_set, item_triple_set, 0, len(item_list)))
        
        for item, score in zip(item_list, scores):
            item_score_map[item] = score
        
        item_score_pair_sorted = sorted(item_score_map.items(), key=lambda x: x[1], reverse=True)
        item_sorted = [i[0] for i in item_score_pair_sorted]
        
        logging.info(item_sorted[:topK])
